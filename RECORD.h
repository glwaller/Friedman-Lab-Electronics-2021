 ...
 *      Version: v0.0 (March 21, 2022) for MCU firmware version v1.1
 *
...
#define delay_ms(x)     __delay_cycles((long) x* 8000)
...
/* ------ Global Variables ------ */
short int _BLINK = 0;           // Indicates that the system needs to use PWM to manipulate a particular LED
volatile char rxstring[4] = ""; // String buffer to store incoming messages from PC. Modifiable by the UART interrupt service routine.
volatile char ccrstring[7] = "";// Stores incoming configuration message from PC. Used by UART ISR.
volatile int length = 4;        // Length of the string buffer. For LED commands.
